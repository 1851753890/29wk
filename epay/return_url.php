...
if($verify_result) {
	...
	if ($money3!=0) {
	    $cg="用户[{$userrow['user']}]成功充值".$money."软妹；本次赠送".$money3."软妹! ";
	}else {
	    $cg="用户[{$userrow['user']}]成功充值".$money."软妹! ";
	}
    if($_GET['trade_status'] == 'TRADE_FINISHED' || $_GET['trade_status'] == 'TRADE_SUCCESS') {
		if($srow['status']==0 && $srow['money']==$money){
			...
			wlog($userrow['uid'],"在线充值","用户[{$userrow['user']}]在线充值了{$money}软妹",$money);
			if ($money3!=0) {
           	    wlog($userrow['uid'],"充值达标赠送","用户[{$userrow['user']}]充值金额达标赠送{$money3}软妹",$money3);
           	}
			exit("<script language='javascript'>alert('$cg');window.location.href='../index/charge';</script>");
		}else{
			$DB->query("update `qingka_wangke_pay` set `status` ='1',`endtime` ='$date',`trade_no`='{$trade_no}' where `out_trade_no`='{$out_trade_no}'"); 
           	wlog($userrow['uid'],"在线充值","重复刷新--用户[{$userrow['user']}]在线充值了{$money}软妹",$money);
			exit("<script language='javascript'>alert('已充值，请勿重复刷新');window.location.href='../index/charge';</script>");
		}
    }
    else {
